...
public class b2Jpanel1 extends JPanel{
	...
	private void init(){
		...
		ImageIcon i12 = new ImageIcon("picture/Ð¡ËÑË÷.png");
		...
		t1 = new JLabel(documentController.getDocCode("Èë¿âµ¥"));
		t2 = new JLabel("Èë¿âµ¥");
		t3 = new JTextField();
	...
	private void registListener(final b2icwarehousemanui ui,final icwarehousemanJpanel icwarehousemanJpanel,final b2Jpanel1 b2jpanel){
		...
		sousuo.addMouseListener(new MouseAdapter() {
		     ImageIcon i12 = new ImageIcon("picture/Ð¡ËÑË÷.png");
			 ImageIcon i13 = new ImageIcon("picture/Ð¡ËÑË÷ºì.png");
			@Override
			public void mousePressed(MouseEvent arg0) {
				// TODO ×Ô¶¯Éú³ÉµÄ·½·¨´æ¸ù
				if (t7.getText().equals("")) {
					if (t6.getText().equals("")) {
						new failDialog(ui, "Ê§°Ü", true);
					}
					else {
		...
		yesButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if (isFull()) {
					documentController.createBlock(new InBillsPO(t1.getText(), "Èë¿âµ¥", t3.getText(), t4.getText(), account, t5.getText(), Integer.parseInt(t6.getText()),  Integer.parseInt(t7.getText()),  Integer.parseInt(t8.getText()),  Integer.parseInt(t9.getText())));
					icwarehousemanJpanel.remove(b2jpanel);
					icwarehousemanJpanel.add(ui.getOperationJpanel());
					ui.getB1().setEnabled(true);
					icwarehousemanJpanel.repaint();
...
class failDialog extends JDialog{
	...
	private void init(){
		ImageIcon yesIcon=new ImageIcon("picture/µÇÂ¼.png");
		jLabel=new JLabel("ÌîÐ´²»ÍêÕû£¬Çë¼ÌÐøÌîÐ´",jLabel.CENTER);
		jLabel.setForeground(Color.white);
		jLabel.setFont(new Font("Ó×Ô²",Font.BOLD,27));
